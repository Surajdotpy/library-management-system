...
describe('Seats API', () => {
  ...
  let branchOneStudentAId: number;
  let branchOneStudentBId: number;
  let branchTwoStudentId: number;
  let branchOneSeatId: number;
  let branchOneSeatTwoId: number;
  let branchTwoSeatId: number;
  ...
  beforeAll(async () => {
    ...
    branchOneStudentAId = branchOneStudentA.body.data.id as number;
    branchOneStudentBId = branchOneStudentB.body.data.id as number;
    branchTwoStudentId = branchTwoStudent.body.data.id as number;
  ...
  it('should create a seat booking for the selected month', async () => {
    ...
    bookingId = response.body.data.id as number;
  });
  ...
  it('should exclude already-booked students from the eligible student list', async () => {
    const response = await request(app)
      .get(
        `/api/seats/eligible-students?branch_id=1&month=${bookingMonth}&year=${bookingYear}`,
      )
      .set('Authorization', `Bearer ${superadminToken}`);
    ...
    expect(response.body.success).toBe(true);
    expect(
      response.body.data.some((student: { id: number }) => student.id === branchOneStudentAId),
    ).toBe(false);
    expect(
      response.body.data.some((student: { id: number }) => student.id === branchOneStudentBId),
    ).toBe(true);
  });
  ...
  it('should show the selected branch seat as booked for the requested month', async () => {
    ...
    const bookedSeat = response.body.data.find(
      (seat: { id: number }) => seat.id === branchOneSeatId,
    );
  ...
  it('should release an existing booking and make the seat available again', async () => {
    ...
    const releasedSeat = seatsResponse.body.data.find(
      (seat: { id: number }) => seat.id === branchOneSeatId,
    );
