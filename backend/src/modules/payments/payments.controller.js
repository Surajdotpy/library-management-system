...
export async function recordPayment(req, res) {
    try {
        ...
        if (!data.student_id || !data.amount || !data.fee_month || !data.fee_year) {
            return badRequest(res, 'Missing required fields: student_id, amount, fee_month, fee_year');
        }
        ...
        if (data.fee_year < 2020 || data.fee_year > currentYear + 1) {
            return badRequest(res, `Invalid fee_year. Must be between 2020 and ${currentYear + 1}`);
        }
        const user = requireAuthenticatedUser(req.user);
        const branchId = resolveAuthorizedBranchId(user);
        const payment = await paymentService.recordPayment(data, user.userId, branchId);
        res.status(201).json({
            success: true,
...
export async function getStudentPayments(req, res) {
    try {
        ...
        const limit = req.query.limit ? Number.parseInt(req.query.limit, 10) : 12;
        const user = requireAuthenticatedUser(req.user);
        const branchId = resolveAuthorizedBranchId(user);
        const history = await paymentService.getStudentPaymentHistory(studentId, limit, branchId);
        res.status(200).json({
            success: true,
...
export async function getPaymentByReceipt(req, res) {
    try {
        ...
        const user = requireAuthenticatedUser(req.user);
        const branchId = resolveAuthorizedBranchId(user);
        const payment = await paymentService.getPaymentByReceiptNumber(receiptNumber, branchId);
        if (!payment) {
