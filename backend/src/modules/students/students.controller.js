...
export async function createStudent(req, res) {
    try {
        const user = requireAuthenticatedUser(req.user);
        const { name, phone, email, study_plan, branch_id, date_of_birth, gender, blood_group, address, city, state, pincode, emergency_contact_name, emergency_contact_phone, emergency_contact_relation, id_proof_type, id_proof_number, } = req.body;
        const requestedBranchId = typeof branch_id === 'string' ? Number.parseInt(branch_id, 10) : branch_id;
        ...
        const normalizedBranchId = resolveAuthorizedBranchId(user, requestedBranchId);
...
export async function updateStudent(req, res) {
    try {
        ...
        const user = requireAuthenticatedUser(req.user);
        const branchId = resolveAuthorizedBranchId(user);
        const updatedStudent = await studentService.updateStudent(id, updateData, branchId);
        if (!updatedStudent) {
            return res.status(404).json({
                success: false,
                error: 'Student not found',
