...
function parseStudentSequence(studentId) {
    if (!studentId) {
        return 0;
    }
...
async function generateStudentId(client, branchId) {
...
export async function getAllStudents(branchId) {
    const values = [];
    let query = `
    SELECT *
    FROM students
...
export async function getStudentById(id, branchId) {
    const values = [id];
    let query = `
    SELECT *
    FROM students
...
export async function createStudent(data) {
    ...
    `);
        ...
        const result = await client.query(`INSERT INTO students (
...
export async function updateStudent(id, data, branchId) {
    let monthlyFee;
    let dailyHoursLimit;
    if (data.study_plan) {
        const pricing = PRICING[data.study_plan];
...
export async function deleteStudent(id, branchId) {
    const values = [id];
    let query = `
    UPDATE students
