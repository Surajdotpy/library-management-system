...
async function getStudentStats(branchId) {
    const params = [];
    let query = `
    SELECT
...
async function getTodayRevenue(branchId) {
    const params = [];
    let query = `
...
async function getRecentPayments(branchId) {
    const params = [];
    let query = `
    SELECT
      p.id AS payment_id,
...
async function getStudentsInside(branchId) {
    const params = [];
    let query = `
    SELECT
...
export async function getDashboardSummary(branchId) {
    ...
    const [studentStats, attendanceSummary, monthlyRevenue, pendingPayments, todayRevenue, recentPayments, studentsInside, branchOverview,] = await Promise.all([
    ...
    const occupancyRate = totalCapacity > 0
        ? Number.parseFloat(((attendanceSummary.currently_inside / totalCapacity) * 100).toFixed(2))
        : 0;
    const summary = {
