...
function resolveBookingPeriod(req) {
    const currentDate = new Date();
    const month = parsePositiveInteger(typeof req.query.month === 'string' ? req.query.month : undefined, currentDate.getMonth() + 1);
    const year = parsePositiveInteger(typeof req.query.year === 'string' ? req.query.year : undefined, currentDate.getFullYear());
    if (month == null ||
        year == null ||
        Number.isNaN(month) ||
        Number.isNaN(year) ||
...
export async function getEligibleStudents(req, res) {
    try {
        ...
        const user = requireAuthenticatedUser(req.user);
        const branchId = resolveAuthorizedBranchId(user, requestedBranchId);
        const students = await seatsService.getEligibleStudents(period.month, period.year, branchId);
        res.status(200).json({
            success: true,
...
export async function getSeatBookings(req, res) {
    try {
        const period = resolveBookingPeriod(req);
        if (!period) {
            return badRequest(res, 'A valid month and year are required');
        ...
        const requestedBranchId = parseBranchId(req.query.branch_id);
        const limit = parsePositiveInteger(typeof req.query.limit === 'string' ? req.query.limit : undefined, 50);
        if (Number.isNaN(requestedBranchId)) {
            return badRequest(res, 'Invalid branch ID');
        }
        ...
        const user = requireAuthenticatedUser(req.user);
        const branchId = resolveAuthorizedBranchId(user, requestedBranchId);
        const bookings = await seatsService.getSeatBookings(period.month, period.year, branchId, Math.min(limit, 200));
        res.status(200).json({
            success: true,
...
export async function createSeatBooking(req, res) {
    try {
        const user = requireAuthenticatedUser(req.user);
        const { seat_id, student_id, booking_month, booking_year, notes } = req.body;
        if (!seat_id || !student_id || !booking_month || !booking_year) {
            return badRequest(res, 'Missing required fields: seat_id, student_id, booking_month, booking_year');
        }
        ...
        if (error.message.includes('selected month') ||
...
export async function releaseSeatBooking(req, res) {
    try {
        ...
        const user = requireAuthenticatedUser(req.user);
        const { release_reason, status } = req.body;
        ...
        const booking = await seatsService.releaseSeatBooking(bookingId, releaseData, user.userId, branchId);
        if (!booking) {
