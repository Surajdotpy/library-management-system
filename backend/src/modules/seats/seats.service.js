...
function getPeriodComparison(month, year) {
...
async function expireOutdatedBookings(queryExecutor) {
...
async function getSeatBookingById(queryExecutor, bookingId) {
...
export async function getSeats(filters) {
    ...
    const params = [filters.booking_month, filters.booking_year];
    let query = `
...
export async function getEligibleStudents(bookingMonth, bookingYear, branchId) {
    await expireOutdatedBookings(pool);
    const params = [bookingMonth, bookingYear];
    let query = `
    SELECT
      s.id,
      s.student_id,
...
export async function getSeatBookings(bookingMonth, bookingYear, branchId, limit = 50) {
    await expireOutdatedBookings(pool);
    const params = [bookingMonth, bookingYear];
    let query = `
    SELECT
...
export async function createSeatBooking(data, assignedBy, branchId) {
    ...
    `;
      ...
      `;
        ...
        const seatResult = await client.query(seatQuery, seatParams);
...
export async function releaseSeatBooking(bookingId, data, releasedBy, branchId) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await expireOutdatedBookings(client);
