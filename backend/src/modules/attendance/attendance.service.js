...
async function verifyStudentAccess(studentId, branchId) {
...
export async function markEntry(data, markedBy, branchId) {
...
export async function markExit(data, markedBy, branchId) {
    const today = new Date().toISOString().slice(0, 10);
    await verifyStudentAccess(data.student_id, branchId);
    const params = [data.student_id, today];
    let findQuery = `
    SELECT a.*
    FROM attendance a
...
export async function getStudentAttendanceHistory(studentId, startDate, endDate, limit = 30, branchId) {
    const params = [studentId];
    let query = `
    SELECT a.*
    FROM attendance a
...
export async function getStudentAttendanceStats(studentId, month, year, branchId) {
    const currentDate = new Date();
    const targetMonth = month || currentDate.getMonth() + 1;
    const targetYear = year || currentDate.getFullYear();
    const params = [studentId, targetMonth, targetYear];
    let query = `
    SELECT
      COUNT(*) as total_days,
